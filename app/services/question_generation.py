...
class QuestionGeneration:
    ...
    def talk_to_npc(self, npc_name, keyword=None, keyword_type=None):
        ...
        # 표시용 이름 가져오기
        display_name = get_name(npc["name"], lang, self.names)
        ...
        if keyword_type == "weapon":
            ...
            if keyword in npc["preferredWeapons"]:
                if is_murderer:
                    response_prompt = (
                        f"Generate a brief, concise response in {lang} (without any greeting) for an NPC named {display_name} with the personality '{npc['personality']}' "
                        ...
                        f"Keep the response concise and natural - aim for about 2/3 of a typical response length. "
                        f"IMPORTANT: Always complete your response with a full, natural sentence. Do not cut off mid-sentence."
                    )
                else:
                    response_prompt = (
                        f"Generate a brief, concise response in {lang} (without any greeting) for an NPC named {display_name} with the personality '{npc['personality']}' "
                ...
                response_prompt = (
                    f"Generate a brief, concise response in {lang} (without any greeting) for an NPC named {display_name} with the personality '{npc['personality']}' "
                    f"and feature '{npc['feature']}' about the weapon '{weapon_name}'. Start directly with their opinion or experience. "
                    f"They strongly dislike or never use this weapon. Have them explain briefly (1-2 sentences) why they avoid it, "
        ...
        # 장소에 대한 답변 생성
            ...
            if keyword in npc["preferredLocations"]:
                ...
                response_prompt = (
                    f"Generate a brief, concise response in {lang} (without any greeting) for an NPC named {display_name} with the personality '{npc['personality']}' "
                    f"and feature '{npc['feature']}' about the location '{location_name}'. Start directly with their opinion or experience. "
                    f"They actively avoid or strongly dislike this place. Have them explain briefly (1-2 sentences) their aversion, "
            ...
            if keyword in npc["preferredTimes"]:  # 깨어있던 시간
                ...
                if is_murderer:
                    response_prompt = (
                        ...
                        f"They claim to have been sleeping, but their response should be brief (1-2 sentences). Include their usual sleep routine, "
                    ...
                    response_prompt = (
                        f"Generate a brief, concise response in {lang} (without any greeting) for an NPC named {display_name} with the personality '{npc['personality']}' "
                        f"and feature '{npc['feature']}' about '{time_name}'. Start directly with their opinion or experience. "
                        f"They were sleeping at this time. Have them explain briefly (1-2 sentences) about their sleep habits, "
